...
def generate_maze(size=27, obstacle_density=0.3, gauss_placement=False, rand_goal=True):
    ...
    for i in range(0, mx):
        for j in range(0, my):
            ...
            if (i==start_x and j==start_y) or (i==goal_x and j==goal_y) or (i==key_x and j==key_y) or (i==11 and j==14):
                pass
    ...
    maze_tensor[PIT][10][14] = 1.0
    maze_tensor[PIT][12][14] = 1.0
    maze_tensor[PIT][11][13] = 1.0
    maze_tensor[PIT][11][15] = 1.0
...
if __name__ == "__main__":
    ...
    env = PitWorld_Key(size=18, one_hot_features=True, rand_goal=False)
    goal_space = [212,  160, 163]
    G = Goal_Space(goal_space=goal_space, grid_size=env.size)
    ...
    print(env.to_string(goal_space=goal_space))
    print(env.agent_pos)
