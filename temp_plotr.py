...
for i in range(no_experiments):
    ...
    R_HRL_60.append(torch.load("results/grid/safe_cost_alloc_lower_hrl_sarsa_key/new/r" + str(i + 1))[:l])
    C_HRL_60.append(torch.load("results/grid/safe_cost_alloc_lower_hrl_sarsa_key/new/c" + str(i + 1))[0:l])

    #R_HRL_90.append(torch.load("results/grid/safe_cost_alloc_lower_hrl_sarsa_key/new/r" + str(i + 1))[:l])   #RELEVANT
    #C_HRL_90.append(torch.load("results/grid/safe_cost_alloc_lower_hrl_sarsa_key/new/c" + str(i + 1))[0:l])



    R_HRL_G_60.append(torch.load("results/grid/safe_global_hrl_sarsa_key/60/r" + str(i + 1))[:l])
    C_HRL_G_60.append(torch.load("results/grid/safe_global_hrl_sarsa_key/60/c" + str(i + 1))[0:l])


    #RELEVANTT

    # AUTOMATIC ALLOCATION
    R_90.append(torch.load("results/grid/safe_cost_alloc_lower_hrl_sarsa_key/new/r" + str(i + 1))[:l])
    C_90.append(torch.load("results/grid/safe_cost_alloc_lower_hrl_sarsa_key/new/c" + str(i + 1))[0:l])
    ...
    #GLOBAL BVF
    R_HRL_G_90.append(torch.load("results/grid/safe_global_hrl_sarsa_key/90/r" + str(i + 1))[:l])    #RELEVANT
    C_HRL_G_90.append(torch.load("results/grid/safe_global_hrl_sarsa_key/90/c" + str(i + 1))[0:l])

    #UNSAFE
    R_k.append(torch.load("results/grid/hrl_sarsa_key/r" + str(i + 1))[:l])    #RELEVANT
...
legend = ["unsafe_hrl", "hrl-safe-cost-alloc", "hrl-safe-global-bvf", "hrl-safe-manual_lower_alloc"]
...
plt.plot(x, R_k_mean, label=legend[0], linewidth=5)
plt.fill_between(x, R_k_mean + R_k_std, R_k_mean - R_k_std, alpha = 0.1)

plt.plot(x, R_mean_90, label=legend[1], linewidth=5)
...
plt.plot(C_k_mean, label=legend[0], linewidth=3)
plt.fill_between(x, C_k_mean + C_k_std, C_k_mean - C_k_std, alpha = 0.3)

plt.plot(C_mean_90, label=legend[1], linewidth=3)
