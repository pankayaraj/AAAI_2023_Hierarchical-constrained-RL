...
for i in range(no_experiments):
    R_k.append(torch.load("results/puddle/sarsa/r" + str(i + 1))[:l1])  # non safe non HRL
    C_k.append(torch.load("results/puddle/sarsa/c" + str(i + 1))[0:l1])

    R_k_HRL.append(torch.load("results/puddle/hrl_sarsa/r" + str(i + 1))[:l1])  # non safe HRL
    C_k_HRL.append(torch.load("results/puddle/hrl_sarsa/c" + str(i + 1))[0:l1])

    R_60.append(torch.load("results/puddle/safe_sarsa/r" + str(i + 1))[:l])  # bvf sarsa
    C_60.append(torch.load("results/puddle/safe_sarsa/c" + str(i + 1))[0:l])

    R_HRL_60.append(torch.load("results/puddle/safe_upper_bvf_lower_lagrangian/r" + str(i + 1))[:l])  # lower level lagrangian
    C_HRL_60.append(torch.load("results/puddle/safe_upper_bvf_lower_lagrangian/c" + str(i + 1))[0:l])

    R_lyp_60.append(torch.load("results/puddle/lyp_sarsa/r" + str(i + 1))[:l])  # bvf sarsa
    C_lyp_60.append(torch.load("results/puddle/lyp_sarsa/c" + str(i + 1))[0:l])
...
name = "figures/puddle/comp/Rewards_Key_30"
...
ax2.axhline(y=20, color='r', linestyle='--',  linewidth=10)
...
name = "figures/puddle/comp/Puddle_30"
plt.savefig(name, bbox_extra_artists=(lgd,), bbox_inches='tight')
plt.close(fig)