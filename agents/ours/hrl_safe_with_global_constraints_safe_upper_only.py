...
class HRL_Discrete_Safe_Global_Constraint(object):
    ...
    def safe_deterministic_pi_upper(self, state, goal, goal_discrete, current_cost=0.0, greedy_eval=False):
        """
        take the action based on the current policy
        ...
        with torch.no_grad():
            ...
            if (random.random() > self.eps_l) or greedy_eval:
                ...
                # Q_D(s,a)
                cost_q_val = self.cost_lower_model(state_goal)
                ###SINCE COST IS ONLY KEPT AT THE UPPER LEVEL######
                action = self.pi_lower(state, goal)

                action = torch.LongTensor(action).unsqueeze(1).to(self.device)
                cost_q_val = cost_q_val.gather(0, action[0])
                ...
                quantity_1 = cost_q_val_upper + cost_r_val_upper
                quantity_2 = self.args.d0 + (cost_q_val)
