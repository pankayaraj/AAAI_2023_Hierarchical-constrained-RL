...
class HRL_Discrete_Goal_SarsaAgent(object):
    ...
    def safe_deterministic_pi_lower(self, state,d_low,  current_cost=0.0, greedy_eval=False):
        """
        take the action based on the current policy
        d_low: Allocated cost for the layer (equivalent to d0 in normal case)
        """
        ...
        with torch.no_grad():
            ...
            if (random.random() >  self.eps_l) or greedy_eval:
                ...
                constraint_mask = torch.le(cost_q_val + cost_r_val, d_low + current_cost).float()
    ...
    def set_policy(self):
        self.policy_lower = self.safe_deterministic_pi_lower
        self.policy_upper = self.pi_meta
