...
for i in range(no_experiments):
    R_k.append(torch.load("results/grid/hrl_sarsa_key/7_sub_goals/r" + str(i + 1))[:l1])  # non safe HRL
    C_k.append(torch.load("results/grid/hrl_sarsa_key/7_sub_goals/c" + str(i + 1))[0:l1])
    ...
    R_HRL_60.append(torch.load("results/grid/safe_upper_bvf_lower_lagrangian/7_sub_goals/new_60/r" + str(i + 1))[:l])   #lower level lagrangian
    C_HRL_60.append(torch.load("results/grid/safe_upper_bvf_lower_lagrangian/7_sub_goals/new_60/c" + str(i + 1))[0:l])

    R_HRL_90.append(torch.load("results/grid/safe_upper_bvf_lower_lagrangian/7_sub_goals/new_90/r" + str(i + 1))[:l])  # lower level lagrangian
    C_HRL_90.append(torch.load("results/grid/safe_upper_bvf_lower_lagrangian/7_sub_goals/new_90/c" + str(i + 1))[0:l])
...
legend = ["unsafe-hrl", "bvf-sarsa, limit = 90", "HiLite, Limit = 90", "global-bvf-safe-hrl", "bvf-sarsa, limit = 60", "HiLite, Limit = 60",
          "lpy-sarsa, limit = 60", "lpy-sarsa, limit = 90"]
...
lgd = plt.legend( prop={'size':88},loc='upper center', bbox_to_anchor=(0.5, -0.05),
          fancybox=True, shadow=True, ncol=3)
plt.xlabel('No of episodes X1000', size=90)
plt.ylabel("Reward", size=90)
plt.title("Reward comparision with a constriant limis of 60, 90 for 7 goals", size=90)
name = "figures/grid/comp/Rewards_Key_90_7_subgoal"
...
plt.plot(C_mean_60, label=legend[4], linewidth=8)
plt.fill_between(x, C_mean_60 + C_std_60, C_mean_60 - C_std_60, alpha = 0.3)

plt.plot(C_mean_90, label=legend[1], linewidth=8)
plt.fill_between(x, C_mean_90 + C_std_90, C_mean_90 - C_std_90, alpha = 0.3)

plt.plot(C_mean_HRL_60, label=legend[5], linewidth=8)
plt.fill_between(x, C_mean_HRL_60 + C_std_HRL_60, C_mean_HRL_60 - C_std_HRL_60, alpha = 0.3)

plt.plot(C_mean_HRL_90, label=legend[2], linewidth=8)
plt.fill_between(x, C_mean_HRL_90 + C_std_HRL_90, C_mean_HRL_90 - C_std_HRL_90, alpha = 0.3)

plt.plot(C_lyp_mean_60, label=legend[6], linewidth=8)
plt.fill_between(x, C_lyp_mean_60 + C_lyp_std_60, C_lyp_mean_60 - C_lyp_std_60, alpha=0.3)

plt.plot(C_lyp_mean_90, label=legend[7], linewidth=8)
...
plt.title("Constraint comparision with a constriant limis of 60, 90 for 7 goals", size=90)
name = "figures/grid/comp/Constraints_Key_90_7_subgoal"
