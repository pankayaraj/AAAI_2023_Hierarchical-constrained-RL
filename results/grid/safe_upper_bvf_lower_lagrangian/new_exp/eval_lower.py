...
for i in eps_of_interest:
    ...
    for k in range(3):
        ...
        q_upper = agent.cost_upper_model(state)
        q_upper = q_upper.gather(0, goal[0])

        if k == 0:
            goal1.append(q_upper.item())
        elif k == 1:
            goal2.append(q_upper.item())
        elif k == 2:
            goal3.append(q_upper.item())
...
plt.plot(goal2)
plt.plot(goal3)
plt.legend(["random", "key", "goal"])
plt.title("Upper Level's  Cost Q Value function for different goals")
plt.show()